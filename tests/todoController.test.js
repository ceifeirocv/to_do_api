import {listTodos, getTodo, deleteTodo, createTodo, updateTodo} from '../controllers/TodoController.js';
...
describe('createTodo Controller', () => {
  ...
  it('if no tilte should ask for a one', async () => {
    ...
    await createTodo(req,res);
    expect(res.statusCode).toBe(400);
    expect(res._getJSONData()).toEqual({"message":"Provide a Title"});
  });
  it('if no description should ask for a one', async () => {
    ...
    await createTodo(req,res);
    expect(res.statusCode).toBe(400);
    expect(res._getJSONData()).toEqual({"message":"Provide a Description"});
  });
  it('if title have less than 5 char should inform, ', async () => {
    ...
    await createTodo(req,res);
    expect(res.statusCode).toBe(400);
    expect(res._getJSONData()).toEqual({ "message": "Title must contain 5 to 50 character" });
  });
  ...
  it('if descritpion have less than 20 char should inform, ', async () => {
    ...
    await createTodo(req,res);
    expect(res.statusCode).toBe(400);
    expect(res._getJSONData()).toEqual({ "message": "Description must contain 20 to 250 character" });
  });
  ...
  describe('updateTodo Controller', () => {
    ...
    it('in_progress should be boolean, ', async () => {
      ...
      await updateTodo(req,res);
      expect(res.statusCode).toBe(400);
      expect(res._getJSONData()).toEqual({ "message": "Status must be true or false"});
    });
    it('in_progress should be boolean, ', async () => {
      await db.query('INSERT INTO todos(title, description) VALUES($1, $2)', ["Lorem", "Sed ut perspiciatis."])
      req = {
